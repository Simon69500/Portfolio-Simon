...
/**
 * @typedef {Object} ProjectRoleEntry
 * @property {string} title
 * @property {string} description
 */

/**
 * Forme de `details` pour le projet phare (isFeatured: true) : chaque section narrative
 * (aspectsTechniques, defis, solutions, resultats) est un OBJET `{ intro, points: [] }`
 * plutôt qu'une simple chaîne, pour permettre l'affichage enrichi en liste à puces dans
 * StarNarrative.jsx. Le projet phare a aussi des champs absents des projets standards :
 * `demoMedia`, `metrics`, `role` (consommés respectivement par StarDemoMedia.jsx,
 * StarMetrics.jsx, StarRole.jsx).
 *
 * @typedef {Object} FeaturedProjectDetails
 * @property {{src: string, poster: string}} demoMedia
 * @property {string} contexte
 * @property {{intro: string, points: string[]}} aspectsTechniques
 * @property {{intro: string, points: string[]}} defis
 * @property {{intro: string, points: string[]}} solutions
 * @property {{intro: string, points: string[]}} resultats
 * @property {{total: ProjectMetric, origine: ProjectMetric[]}} metrics
 * @property {ProjectRoleEntry[]} role
 */

/**
 * Forme de `details` pour un projet standard (isFeatured: false) : les mêmes 4 sections
 * narratives existent, mais en CHAÎNES DE TEXTE SIMPLES — pas d'objet { intro, points }.
 * C'est ProjetDetails.jsx (et non ProjetDetailsStar.jsx) qui consomme cette forme, en
 * affichant chaque champ tel quel dans un bloc de texte.
 *
 * @typedef {Object} StandardProjectDetails
 * @property {string} contexte
 * @property {string} aspectsTechniques
 * @property {string} defis
 ...
 * @property {number} id
 * @property {boolean} isFeatured - true pour un seul projet à la fois : celui-ci va dans
 *   la tuile D (ProjetStar, pleine largeur) plutôt que dans la mosaïque des tuiles F/G.
 *   Voir Home.jsx : `portfolioData.find(p => p.isFeatured === true)`.
 * @property {'professionnel'|'formation'|'personnel'} type - clé de correspondance avec
 *   projectTypeData.js, pour le badge affiché en tête de carte.
 * @property {string} titre
 * @property {string} slug
 * @property {string} coverImage - chemin construit via `img()`.
 * @property {string[]} gallery - chemins construits via `img()`, consommés par
 *   ProjectCarousel.jsx.
 * @property {string[]} technologies - noms de technologies en texte libre. Rapprochés des
 *   entrées de techData.js UNIQUEMENT à l'affichage (par comparaison de `name`, insensible
 *   à la casse) — une faute de frappe ici n'empêche rien de fonctionner, mais empêche le
 *   badge technique de retrouver le bon logo (voir `getTechDetails` dans ProjetStar.jsx et
 *   ProjetCard.jsx, qui affiche alors un badge texte neutre en repli).
 * @property {FeaturedProjectDetails|StandardProjectDetails} details - la forme exacte
 *   dépend de `isFeatured` (voir les deux typedefs ci-dessus).
